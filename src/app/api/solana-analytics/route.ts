...
export async function GET() {
  ...
  if (cached) {
    ...
    if (cacheAge > MAX_CACHE_AGE_MS) {
      console.warn(
        `⚠️  Solana cache is stale (${cacheAgeHours} hours old, max ${MAX_CACHE_AGE_MS / (1000 * 60 * 60)} hours). Falling back to direct fetch.`,
      );
      cached = undefined; // Force fallback to fresh data
    } else {
      console.log(`✅ Serving fresh Solana cached data (${cacheAgeHours} hours old)`);
    }
  }
