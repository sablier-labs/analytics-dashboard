...
export const SolanaMAU = memo(function SolanaMAU() {
  const { data, isLoading, error } = useSolanaAnalytics();
  const mau = data?.mau || null;
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-text-secondary">Monthly Active Users (Solana)</p>
          <SourceCodeLink
            fileName="solana-graphql.ts"
            lineNumber={1}
            tooltip="View fetchSolanaMAU source"
          />
        </div>
        <SharePanel
          title="Monthly Active Users (Solana)"
          elementRef={containerRef}
          description="Monthly active users on the Sablier Solana protocol"
        />
      </div>
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <p className="text-2xl font-bold text-text-primary">
            {mau !== null ? formatNumber(mau) : "—"}
          </p>
        </div>
