...
export const SolanaTotalUsers = memo(function SolanaTotalUsers() {
  ...
  if (error) {
    return (
      <div className="bg-white dark:bg-bg-secondary rounded-xl border border-red-300 dark:border-red-600 shadow-lg p-6 transition-all duration-200 ">
        <p className="text-sm text-red-600 dark:text-red-400 mb-2">Error loading total users</p>
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-text-secondary">Total Users (Solana)</p>
          <SourceCodeLink
            fileName="solana-graphql.ts"
            lineNumber={1}
            tooltip="View fetchSolanaTotalUsers source"
          />
        </div>
        <SharePanel
          title="Total Users (Solana)"
          elementRef={containerRef}
          description="Total unique users on the Sablier Solana protocol"
        />
      </div>
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <p className="text-2xl font-bold text-text-primary">
            {totalUsers !== null ? formatNumber(totalUsers) : "—"}
          </p>
        </div>
