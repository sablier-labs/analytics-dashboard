...
export const CumulativeUserChart = memo(function CumulativeUserChart() {
  ...
  const chartData = useMemo(() => {
    if (!userGrowthData) return { datasets: [], labels: [] };
  ...
  }, [userGrowthData, theme, formatMonth]);
  ...
  if (!userGrowthData || userGrowthData.length === 0) {
    return (
      <div className="bg-white dark:bg-bg-secondary rounded-xl border border-border-default shadow-lg p-6 transition-all duration-200">
        <p className="text-text-secondary">No user growth data available</p>
      </div>
    );
  }
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-bold text-text-primary">Cumulative User Growth</h2>
            <SourceCodeLink
              fileName="graphql.ts"
              lineNumber={381}
